...
describe('UrlHandler', () => {
    ...
    test('getTopContributors should fetch and store top 3 contributors commits', async () => {
        ...
        // Mock axios.get for contributors
        (axios.get as jest.Mock).mockResolvedValueOnce(mockContributors);

        // Mock axios.get for commits per contributor
        (axios.get as jest.Mock).mockResolvedValue(mockCommits);

        await urlHandler.getTopContributors('owner', 'repo');

        // Total commits from top 3 contributors in the past year
        expect(urlHandler['commitsMap'].get('top3')).toBe(30); // Adjusted expected result
    });
    ...
    test('getClosedIssues should fetch and store closed issues counts', async () => {
        ...
        expect(urlHandler['commitsMap'].get('issuesClosed6mth')).toBe(10);
    ...
    // Test for getCodeReviewFraction
    describe('getCodeReviewFraction', () => {
        it('should calculate the fraction of code introduced through PRs with code reviews', async () => {
            ...
            const mockPRDetailsResponses = [
                { data: { additions: 100 } }, // PR 1
                { data: { additions: 200 } }, // PR 2
                { data: { additions: 300 } }, // PR 3
            ];

            const mockReviewsResponses = [
                { data: [{ id: 1 }] }, // PR 1 has reviews
                { data: [] },          // PR 2 has no reviews
                { data: [{ id: 2 }] }, // PR 3 has reviews
            ];

            // Mock axios.get calls in the order they are expected to be called
            // First, the search for PRs
            (axios.get as jest.Mock).mockResolvedValueOnce(mockPullRequestsResponse);

            // Then, the PR details and reviews for each PR
            (axios.get as jest.Mock)
                .mockResolvedValueOnce(mockPRDetailsResponses[0]) // PR 1 details
                .mockResolvedValueOnce(mockReviewsResponses[0])   // PR 1 reviews
                .mockResolvedValueOnce(mockPRDetailsResponses[1]) // PR 2 details
                .mockResolvedValueOnce(mockReviewsResponses[1])   // PR 2 reviews
                .mockResolvedValueOnce(mockPRDetailsResponses[2]) // PR 3 details
                .mockResolvedValueOnce(mockReviewsResponses[2]);  // PR 3 reviews

            await urlHandler['getCodeReviewFraction']('owner', 'repo');

            // Total additions: 100 + 200 + 300 = 600
            // Reviewed additions: PR 1 (100) + PR 3 (300) = 400
            // Expected fraction: 400 / 600 = 0.6667

            const fraction = urlHandler['commitsMap'].get('CodeReviewFraction');
            expect(fraction).toBeCloseTo(0.6667, 4);
        });

        it('should handle zero total additions', async () => {
            const mockPullRequestsResponse = {
                data: {
                    items: [],
                    total_count: 0,
        ...
        it('should handle API errors gracefully', async () => {
            ...
            const fraction = urlHandler['commitsMap'].get('CodeReviewFraction');
            expect(fraction).toBe(0); // Since totalAdditions is 0

            expect(consoleSpy).toHaveBeenCalledWith('Error fetching pull requests:', 'API error');
    ...
    describe('getRepoMetrics', () => {
        it('should fetch all metrics and update the database', async () => {
            ...
            jest.spyOn(urlHandler as any, 'checkLicense').mockResolvedValue(null);
            jest.spyOn(urlHandler as any, 'getOpenedIssues').mockResolvedValue(null);
            jest.spyOn(urlHandler as any, 'getCodeReviewFraction').mockResolvedValue(null);
