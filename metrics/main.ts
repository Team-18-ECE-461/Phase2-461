...
manager.registerCommand('test', 'Test suite', () => {
    ...
    /*
    exec('npx jest --coverage --detectOpenHandles --json --outputFile=jest-results.json --coverageReporters="json-summary"',
        (error, stdout, stderr) => {
            if (error) {
            ...
            try {
                // Read the Jest results from the JSON file
                const jestResults = JSON.parse(fs.readFileSync('jest-results.json', 'utf8'));
                ...
                // Read the coverage summary from the JSON file
                const coverageSummary = JSON.parse(fs.readFileSync('coverage/coverage-summary.json', 'utf8'));
