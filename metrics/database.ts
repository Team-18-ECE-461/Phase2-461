...
export function addEntry(db: Database.Database, url: string, fp: number, loglvl: number, information?: string, metrics?: string): void {
     ...
     * - None
     */
    let stmt = db.prepare(`DELETE FROM package_scores`);
    stmt.run(); // Execute the query to delete all rows
    stmt = db.prepare(`INSERT INTO package_scores (url, information, metrics) VALUES (?, ?, ?)`);
